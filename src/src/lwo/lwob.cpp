...
lwSurface *lwGetSurface5( picoMemStream_t *fp, int cksize, lwObject *obj ){
	...
	while ( 1 ) {
		...
		case ID_BTEX:
			s = reinterpret_cast<decltype(s)>(getbytes( fp, sz ));
			tex = get_texture( s );
		...
		case ID_TREF:
			...
			tex->tmap.ref_object = reinterpret_cast<decltype(tex->tmap.ref_object)>(getbytes( fp, sz ));
			break;
		...
		case ID_SHDR:
			...
			shdr->name = reinterpret_cast<decltype(shdr->name)>(getbytes( fp, sz ));
		...
		case ID_SDAT:
			...
			shdr->data = reinterpret_cast<decltype(shdr->data)>(getbytes( fp, sz ));
			break;
